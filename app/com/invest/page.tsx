...
import { ExternalLink, Search, X, AlertTriangle, BookOpen, BarChart2, Shield } from 'lucide-react';
      ...
      >
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 pb-6 border-b border-red-600">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold">
              <span className="text-red-500">DIRECT HORIZON</span> - GUIDE DES PLATEFORMES
            </h1>
            <p className="text-gray-400 mt-2">Connais ton ennemi - Les commissions cachées démasquées</p>
          </div>
          ...
          <motion.button 
            ...
            whileTap={{ scale: 0.95 }}
          >
            {isSearching ? <X className="mr-2" size={18} /> : <Search className="mr-2" size={18} />}
        ...
        <div className="bg-gradient-to-r from-red-900 to-red-800 rounded-xl p-6 mb-8 border border-red-600">
          <div className="flex items-start">
            ...
            <div>
              ...
              <p>
                Les 4 géants (Booking Holdings, Expedia Group, Airbnb et Trip.com) contrôlent <span className="font-bold text-red-300">85%</span> du marché mondial des réservations en ligne. 
                Cette concentration permet des commissions allant jusqu'à <span className="font-bold text-red-300">30%</span> qui sont répercutées sur les voyageurs et les hôteliers.
              </p>
            </div>
          </div>
        </div>
            ...
            >
              <div className="bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 rounded-xl p-6 h-full hover:border-red-500 transition">
                <div className="flex justify-between items-start">
                  <div>
                    <div className="flex items-center">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center mr-3 ${
                        link.risk === 'Élevé' ? 'bg-red-900' : 
                        link.risk === 'Moyen' ? 'bg-amber-900' : 'bg-green-900'
                      }`}>
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-2">
                      ...
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        link.risk === 'Élevé' ? 'bg-red-800 text-red-100' : 
                        link.risk === 'Moyen' ? 'bg-amber-800 text-amber-100' : 'bg-green-800 text-green-100'
                      }`}>
                        Risque: {link.risk}
                      </span>
                    </div>
                  ...
                  <ExternalLink className="text-gray-500 group-hover:text-red-500 transition" size={18} />
                </div>
        ...
        <div className="mt-12 p-6 bg-gradient-to-r from-green-900 to-emerald-800 rounded-xl border border-green-600">
          <div className="flex items-start">
            ...
            <div>
              <h3 className="text-xl font-bold mb-2">La Solution Direct Horizon</h3>
              <p className="mb-4">
                Chez Direct Horizon, nous proposons une alternative radicale avec <span className="font-bold text-green-300">0% de commission</span> 
        ...
        <footer className="mt-12 pt-8 border-t border-gray-800 text-center text-gray-500 text-sm">
          ...
          <p className="mt-2">Données actualisées: {new Date().toLocaleDateString('fr-FR')}</p>
          <p className="mt-1 text-xs text-gray-600">
            *Estimations basées sur des données publiques et des témoignages d'hôteliers
          </p>
        </footer>
      </motion.div>
    </div>
